pro shong_runDetection,plugmap,dTh,dWd, lamArr, signalArr, nSigmaArr
;-----------------------------
...
;-----------------------------
	...
	;
		    ...
		    read, isWrite, prompt='("y" for yes; others for no) :'
			if strupcase(isWrite) eq 'Y' then begin
				...
				save,lam,originalSignalBaseline,normalizedSignalSmooth,noiseSigma,noiseSigmaSmooth,filename=savfilename
			...
			for iL=1L,newDetections[0,0] do begin
				...
				junky = mpfitpeak(lam[idxLower:idxUpper],normalizedSignalSmooth[idxLower:idxUpper],gpara,nterms=3)
                ...
                info = string(format='(i05," ",i02," ",13f10.2," ")',$
